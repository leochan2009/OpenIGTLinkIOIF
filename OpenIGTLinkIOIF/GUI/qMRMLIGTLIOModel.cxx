...
//-----------------------------------------------------------------------------
void qMRMLIGTLIOModel::setLogic(vtkSlicerOpenIGTLinkIOIFLogic* logic)
{
  if (!logic)
    {
    return;
...
QStandardItem* qMRMLIGTLIOModel::insertNode(vtkMRMLNode* node, QStandardItem* parent, int row)
{
  ...
  if (this->listenNodeModifiedEvent() &&
    ...
    {
    qvtkConnect(node, vtkIGTLIOConnector::ConnectedEvent,
                this, SLOT(onMRMLNodeModified(vtkObject*)));
    qvtkConnect(node, vtkIGTLIOConnector::DisconnectedEvent,
                this, SLOT(onMRMLNodeModified(vtkObject*)));
    qvtkConnect(node, vtkIGTLIOConnector::ActivatedEvent,
                this, SLOT(onMRMLNodeModified(vtkObject*)));
    qvtkConnect(node, vtkIGTLIOConnector::DeactivatedEvent,
                this, SLOT(onMRMLNodeModified(vtkObject*)));
    qvtkConnect(node, vtkIGTLIOConnector::NewDeviceEvent,
                this, SLOT(onMRMLNodeModified(vtkObject*)));
    qvtkConnect(node, vtkIGTLIOConnector::DeviceModifiedEvent,
...
void qMRMLIGTLIOModel::updateItemDataFromNode(QStandardItem* item, vtkMRMLNode* node, int column)
{
  ...
  vtkMRMLIGTLConnectorNode* cnode = vtkIGTLIOConnector::SafeDownCast(node);
  if (!cnode)
    {
    return;
    }
...
//------------------------------------------------------------------------------
void qMRMLIGTLIOModel::updateIOTreeBranch(vtkIGTLIOConnector* node, QStandardItem* item, qMRMLIGTLIOModel::Direction dir)
{
  // List incoming data
  if (node == NULL)
    {
