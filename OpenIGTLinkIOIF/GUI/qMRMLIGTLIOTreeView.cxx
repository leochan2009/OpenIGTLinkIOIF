...
public:
  ...
  vtkSlicerOpenIGTLinkIOIFLogic*      Logic;
};
...
int qMRMLIGTLIOTreeView::rowProperty(const QModelIndex& index, vtkIGTLIOConnector* &cnode, int & dir, vtkMRMLNode* &dnode)
{
  Q_D(qMRMLIGTLIOTreeView);
  Q_ASSERT(d->SortFilterModel);
  ...
  cnode = NULL;
  dnode = NULL;
  dir = vtkIGTLIOConnector::IO_UNSPECIFIED;
  ...
  if (text.compare(QString("IN")) == 0)
    ...
    cnode = vtkIGTLIOConnector::SafeDownCast(node);
    dir = vtkIGTLIOConnector::IO_INCOMING;
    return TYPE_STREAM;
    }
    ...
    vtkMRMLNode* node = sceneModel->mrmlNodeFromIndex(parent1->index());
    cnode = vtkIGTLIOConnector::SafeDownCast(node);
    dir = vtkIGTLIOConnector::IO_OUTGOING;
    return TYPE_STREAM;
    }
...
void qMRMLIGTLIOTreeView::onClicked(const QModelIndex& index)
{
  ...
  if (index.column() == qMRMLIGTLIOModel::VisualizationColumn)
    ...
    if (dnode && type == TYPE_DATANODE)
      ...
      cnode->InvokeEvent(vtkIGTLIOConnector::DeviceModifiedEvent);
      }
    emit ioTreeViewUpdated(type, cnode, dir, dnode);
    }
  else if (index.column() == qMRMLIGTLIOModel::PushOnConnectColumn)
    {
    if (dnode && type == TYPE_DATANODE && dir == vtkIGTLIOConnector::IO_OUTGOING)
      {
      // Toggle the checkbox for "push on connect" feature
      const char * attr = dnode->GetAttribute("OpenIGTLinkIOIF.pushOnConnect");
      if (attr && strcmp(attr, "true") == 0)
        {
        dnode->SetAttribute("OpenIGTLinkIOIF.pushOnConnect", "false");
        }
      else
        {
        dnode->SetAttribute("OpenIGTLinkIOIF.pushOnConnect", "true");
        }
      cnode->InvokeEvent(vtkIGTLIOConnector::DeviceModifiedEvent);
      }
    emit ioTreeViewUpdated(type, cnode, dir, dnode);
...
//-----------------------------------------------------------------------------
void qMRMLIGTLIOTreeView::setLogic(vtkSlicerOpenIGTLinkIOIFLogic* logic)
{
  Q_D(qMRMLIGTLIOTreeView);
  if (!logic)
    {
