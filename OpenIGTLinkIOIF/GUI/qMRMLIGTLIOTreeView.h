...
class Q_SLICER_QTMODULES_OPENIGTLINKIOIF_EXPORT qMRMLIGTLIOTreeView : public qMRMLTreeView
{
  Q_OBJECT
...
signals:  
  //void connectorNodeUpdated(vtkIGTLIOConnector*, int);
  void ioTreeViewUpdated(int, vtkIGTLIOConnector*, int, vtkMRMLNode*);
...
protected:
  ...
  // connector node, the direction of stream (either vtkIGTLIOConnector::IO_INCOMING
  // or vtkIGTLIOConnector::IO_OUTGOING), and the pointer to the data node
  // are set to 'cnode', 'dir', and 'dnode' respectively.
  int rowProperty(const QModelIndex& index, vtkIGTLIOConnector* &cnode, int & dir, vtkMRMLNode* &dnode);
...
private:
  ...
  // toggle the visibility of an OpenIGTLinkIOIF
